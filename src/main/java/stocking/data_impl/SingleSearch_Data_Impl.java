...
public class SingleSearch_Data_Impl implements SingleSearch_Data_Service{
    ...
    public StockPO getStockList(String identifier,Date startDate,Date endDate) throws ParseException{
        ...
        if(isInteger){
            String section=getsectionbycode(identifier);
            if(section.length()>0){
                String sql="select date,open,high,close,low,volume,code from kdata_"+section+" where code="+identifier+" and date>"+startDateStr+" and date<"+endDateStr+" and autotype='无复权'";
...
//        boolean x=isInteger("aaaa");
//        boolean y=isInteger("000001");
//        System.out.print(x);
//        System.out.print(y);
        System.out.print("aaaa".substring(0,3));
        System.out.print("aaaa".length());
